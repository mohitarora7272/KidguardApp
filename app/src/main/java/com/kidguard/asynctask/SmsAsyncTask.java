...
public class SmsAsyncTask extends AsyncTask<String, Void, ArrayList<Sms>> implements Constant {
                            ...
                            for (int i = 0; i < smsDao.queryForAll().size(); i++) {
                                if (c.moveToFirst()) {
                                    for (int j = 0; j < totalSMS; j++) {
                                        List<Sms> results = queryBuilder.where().eq(Sms.SMS_ID, c.getString(c.getColumnIndexOrThrow("_id"))).query();
                                        if (results.size() == 0) {
                                            setSmsPOJO(c);
                                        }
                                    }
    ...
    private ArrayList<Sms> smsFetchWithTag(Dao<Sms, Integer> smsDao, String count, String dateFrom, String dateTo) {
        ...
        try {
            if (!count.equals("") && dateFrom.equals("") && dateTo.equals("")) {
                Log.d("1", "1??");
                List<Sms> results = queryBuilder.where().eq(Sms.SMS_STATUS, FALSE).query();
            ...
            if (count.equals("") && dateFrom.equals("") && dateTo.equals("")) {
                Log.d("4", "4??");
                List<Sms> results = smsDao.queryForAll();
                if (results != null && results.size() > 0) {
                    for (int j = 0; j < results.size(); j++) {
                        setSmsPOJO(lstSmsSorted, results, j);
                    }
    ...
    private List<String> getResultedDateFromTo(QueryBuilder<Sms, Integer> queryBuilder, String dateFrom, String dateTo) {
        List<String> sortList = new ArrayList<>();
        try {
            List<Sms> results = queryBuilder.where().between(Sms.SMS_DATE, dateFrom, dateTo).and().eq(Sms.SMS_STATUS, FALSE).query();
            if (results != null && results.size() > 0) {
